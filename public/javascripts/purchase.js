...
(function($) {
    ...
    Pager.prototype = {
        ...
        init: function() {
            var self = this;
			let tableSort = $('#tableSort').find('tbody');
           	let purchase = $('#purchase').find('tbody');
            let result;
                $.ajax({
                    type: "post",
                    url: "/purchase",
                    async: false,
                    data : {limit : 0, maxlimit: this.options.maxlimit, id: self.options.id, sort: self.options.sort},
                    success: function(data){
                        for (let i = 0; i < data.length; i ++) {
                            tableSort.find('.id').text(data[i].id);
                            tableSort.find('.name').text(data[i].name);
                            tableSort.find('.brand').text(data[i].brand);
                            tableSort.find('.num').text(data[i].num);
                            tableSort.find('.amount').text(data[i].amount);
                            tableSort.find('.useraccount').text(data[i].useraccount);
                            tableSort.find('.time').text(data[i].createTime);
                            tableSort.find('.txHash').text(data[i].txHash);
                    
                            purchase.append(tableSort.html());
                      }
...
//              console.warn('(jquery.pagination) Warn: ' + '"' + self.options.styleURL + '"' + '加载失败! ' + '您的页面已存在一个' + '"' + self.options.styleURL.slice(self.options.styleURL.lastIndexOf('/') + 1) + '"' + '文件!');
        ...
        _render: function() {
            ...
            html.push(__template([options.pageTurnClass], Math.min(options.totlePageCount, options.pageIndex), options.nextPageName));
            ...
            self.$elem.on('click', 'a', function(e) {
                ...
                let result;
                	$.ajax({
	                    ...
	                    url: "/purchase",
	                    async: false,
	                    data : {limit : self.options.pageIndex*self.options.maxlimit, maxlimit : self.options.maxlimit, id: self.options.id, sort : self.options.sort},
	                    success: function(data){
	                        for (let i = 0; i < data.length; i ++) {
	                            tableSort.find('.id').text(data[i].id);
	                            tableSort.find('.name').text(data[i].name);
        ...
        _sort: function(){
        	var self = this;
			let result;
			$(".icon-jiantou_up").on('click', function(e) {
				...
				$.ajax({
                    ...
                    url: "/purchase",
                    async: false,
                    data : {limit : 0*self.options.maxlimit, maxlimit : self.options.maxlimit, id: self.options.id, sort : self.options.sort},
                    success: function(data){
                        for (let i = 0; i < data.length; i ++) {
                            tableSort.find('.id').text(data[i].id);
                            tableSort.find('.name').text(data[i].name);
			...
			$(".icon-jiantouxia").on('click', function(e) {
				self.options.pageIndex = 0;
				self.init();
				self.options.sort = 'asc';
				if(e.target.id == 11){
					self.options.id= 'id';
    ...
    $.fn.paginate = function(options, callback) {
    	 $.ajax({
                    ...
                    async: false,
                    success: function(data){
                        if(data[0].count%options.maxlimit !=0){
                        	options.totlePageCount = Math.ceil(data[0].count/options.maxlimit);
                        }else{
                        	options.totlePageCount = Math.floor(data[0].count/options.maxlimit);
                        }
